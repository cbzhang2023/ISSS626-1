...
```{r}
pacman::p_load(sf, spNetwork, tmap, tidyverse, spatstat, lubridate, dplyr, reader, raster, sparr, plotly, ggplot2, spatstat)
```
...
These two plots, using K-function and G-function analyses, reveal the spatial distribution characteristics of traffic accidents in Bangkok. The K-function plot shows that as distance increases, the cumulative number of accident points also rises, with the observed K-function curve clearly exceeding the theoretical random distribution curve. This indicates that traffic accident points in Bangkok exhibit spatial clustering rather than random distribution. The clustering effect persists across varying distances, suggesting higher accident frequency in specific areas, highlighting spatial correlation.

The G-function plot further supports this finding by analyzing the proximity of accident points through nearest-neighbor distances. The actual G-function curve is steeper than the theoretical random distribution curve, indicating that accident points are closer to each other, signifying a higher degree of clustering. This shows that in certain areas of Bangkok, traffic accidents are more concentrated along specific locations or road segments, reinforcing the idea of spatial clustering. Therefore, this spatial pattern may suggest the need for additional traffic safety measures or improvements in road planning in accident-prone areas.
